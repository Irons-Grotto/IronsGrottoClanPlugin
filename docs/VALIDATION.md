...
- [ ] Panel shows a red "Open your collection log in game." Open it → **Sync collection log**
      button. Press it → "Syncing…", then green "Collection log synced." Opening the log alone
      sends nothing.
- [ ] Check the record:
      ```sh
...
- [ ] Open the collection log and press Sync → a full, real-sized log is stored but **not** applied to any player:
      ```sh
...
- [ ] Open the collection log and press Sync in the panel → **Sync your collection log** ticks with
      the slot count; a prospect gets "Back to Irons Grotto".
